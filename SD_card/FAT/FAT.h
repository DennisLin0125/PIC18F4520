...
#define FIX_DIRECTORY 1		/* 1 means use fix directory, 0 for any directory */

#if FIX_DIRECTORY==0

#define  RECORD_ADDR_START 0	/* eeprom start address */
#define  RECORD_ADDR_END  256	/* eeprom end address */
...
extern unsigned long FAT_NextCluster(unsigned long cluster);//查詢下一簇號
extern unsigned int FAT_FindItem(unsigned long cluster, BYTE *name, struct FileInfoStruct *FileInfo);//查詢檔案
extern unsigned long FAT_OpenDir(BYTE * dir);//打開目錄

#if FIX_DIRECTORY
	extern unsigned char Search(BYTE *dir,struct direntry *MusicInfo,WORD *Count);//查詢音樂檔案
#else
	extern BYTE SearchInit(void);
	extern unsigned char Search(struct direntry *MusicInfo,WORD *Count,BYTE *type);//查詢音樂檔案
#endif

extern unsigned char FAT_LoadPartCluster(unsigned long cluster,unsigned part,BYTE * buffer);//載入檔案
